#!/usr/bin/env python3
"""
Evaluate the quadratic form Q associated with Ĵ on code space basis vectors.

For the Hermitian form J with matrix Ĵ, the quadratic form is:
    Q(v) = ⟨v|Ĵ|v⟩
...
Usage:
    python evaluate_Jhat_quadratic_form.py -i Jhat.txt
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate quadratic form Q(v) = ⟨v|Ĵ|v⟩ on code space basis"
    )
    parser.add_argument(
        '-i', '--input',
        required=True,
    ...
    log("=" * 70)
    log(f"Evaluating quadratic form Q(v) = ⟨v|Ĵ|v⟩")
    ...
    # Evaluate Q(|0_L⟩) = ⟨0_L|Ĵ|0_L⟩
    log("Computing Q(|0_L⟩) = ⟨0_L|Ĵ|0_L⟩...")
    bra_0L = ket_0L.H
    Q_0L = (bra_0L * J_hat * ket_0L)[0, 0]
    log("  Simplifying...")
    Q_0L = simplify(Q_0L)
    log("")

    # Evaluate Q(|1_L⟩) = ⟨1_L|Ĵ|1_L⟩
    log("Computing Q(|1_L⟩) = ⟨1_L|Ĵ|1_L⟩...")
    bra_1L = ket_1L.H
    Q_1L = (bra_1L * J_hat * ket_1L)[0, 0]
    log("  Simplifying...")
    Q_1L = simplify(Q_1L)
    log("")
